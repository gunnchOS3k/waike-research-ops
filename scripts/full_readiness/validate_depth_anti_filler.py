...
def empty_section_findings(path: Path, text: str) -> list[dict[str, str]]:
    ...
    if not text.strip():
        ...
        return findings
    # Only flag markdown heading sections with almost no body and no lists/tables.
    ...
    while i + 1 < len(parts):
        ...
        has_structure = bool(
            re.search(r"(?m)^(\|.+\||\s*[-*]\s+\S|\s*\d+\.\s+\S)", body)
        )
...
def main() -> int:
    ...
    if len(readmes) >= 2:
        ...
        for i, a in enumerate(tids):
            ...
            for b in tids[i + 1 :]:
                ...
                if ratio >= 0.75 and len(heavy) >= 12:
                    findings.append(
                        {
                            "severity": "warning",
                            "track_id": f"{a}|{b}",
                            ...
                            "detail": (
                                f"README.md share {len(heavy)} long lines "
                ...
                findings.append(
                    {
                        ...
                        "path": p,
                        "detail": f"Program file contains {len(hits)} known generic phrases",
                    }
                )
    ...
    for skel, items in lab_skeletons.items():
        ...
        if len(tracks_hit) >= 3 and len(names) >= 3:
            # Likely noun-swapped pattern family
            findings.append(
                {
                    "severity": "warning",
