...
from waike_course_ready.batch004.labs import (
...
from waike_course_ready.batch009.labs import (
    LABS_009, COURSE_LABS_009, LAB_SPECS_009, REFERENCE_009, WRONG_009,
)
...
COURSE_LABS.update(COURSE_LABS_009)
...
LAB_SPECS.update(LAB_SPECS_009)
...
REFERENCE.update(REFERENCE_009)
...
def run_all() -> dict[str, Any]:
    ...
    try:
        ...
        print_pass_on_submit_43 = False
    except AssertionError:
        print_pass_on_submit_43 = True
    try:
    ...
    bad_bot_43 = lab_iam_rbac({
        "roles": {"ai.triage.bot": {"actions": ["case.read", "case.close"]}, "analyst": {"actions": ["case.read"]},
                  "lead": {"actions": ["case.close"]}},
        "bindings": {"naiya": "analyst", "omar": "lead", "harbor-bot": "ai.triage.bot"},
    })
    negatives.append({"lab_id": "lab_iam_rbac_negative", "ok": (not bad_bot_43.ok)})
    # #44 package negatives
    ...
    # #46 package negatives
    bad_6g = lab_fspl_budget({"d_m": 120, "f_mhz": 3500, "fspl_db": 1.0, "commercial_6g_exists": True})
    negatives.append({"lab_id": "lab_fspl_budget_negative", "ok": (not bad_6g.ok)})
    bad_estop = lab_estop_policy({"motors_disabled": False, "brake_engaged": False, "resume_requires_human": False})
    ...
    return {
        ...
        "no_submission_fails": no_sub_ok,
        "print_pass_raises": print_ok,
